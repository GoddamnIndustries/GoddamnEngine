            ...
            : base(Dependency)
        {
            if (m_IsSupported) {
                ...
                foreach (TargetPlatform Platform in Target.EnumerateAllPlatforms()) {
                    m_CachedCopyFiles.Add(Platform, Dependency.EnumerateCopyFiles(Platform).ToArray());
                    m_CachedLinkedLibraries.Add(Platform, Dependency.EnumerateCopyFiles(Platform).ToArray());
                }
            }
        }
