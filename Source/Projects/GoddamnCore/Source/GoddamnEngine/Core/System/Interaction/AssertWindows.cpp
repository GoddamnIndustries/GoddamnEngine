...
/*!
 * @file GoddamnEngine/Core/System/Assert/AssertWindows.cpp
 * Desktop Windows error handling algorithms.
 */
//#include <GoddamnEngine/Core/System/Interaction/Assert.h>
...
GD_NAMESPACE_BEGIN
	...
	enum
	{
		GD_ASSERT_WINDOWS_DIALOG_ITEM_ID = 1001,
		GD_ASSERT_WINDOWS_DIALOG_TEXTEDIT_ID = 10,
	};	// enum

	#define GD_ASSERT_WINDOWS_DIALOG_ITEM_CREATE(style, x, y, cx, cy, id, idClass, text) \
		AssertWindowsDialogItem<GD_ARRAY_LENGTH(text)> GD_GLUE(m_DialogItem, __LINE__) \
			= { WS_CHILD | WS_VISIBLE | style, 0, x, y, cx, cy, id != 0 ? GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + id : 0, 0xFFFF, idClass, GD_WIDEN(text), 0 };

	#define GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE(x, y, cx, cy, id, text) \
		GD_ASSERT_WINDOWS_DIALOG_ITEM_CREATE(BS_PUSHBUTTON | WS_TABSTOP, x, y, cx, cy, id, 0x0080, text)

	#define GD_ASSERT_WINDOWS_DIALOG_TEXTEDIT_CREATE(x, y, cx, cy, id, text) \
		GD_ASSERT_WINDOWS_DIALOG_ITEM_CREATE(ES_MULTILINE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL | ES_AUTOVSCROLL | ES_READONLY, x, y, cx, cy, id, 0x0081, text)

	#define GD_ASSERT_WINDOWS_DIALOG_INFOPANEL_CREATE(x, y, cx, cy, id, text) \
		GD_ASSERT_WINDOWS_DIALOG_ITEM_CREATE(BS_GROUPBOX, x, y, cx, cy, id, 0x0080, text)

	#define GD_ASSERT_WINDOWS_DIALOG_STATICTEXT_CREATE(x, y, cx, cy, id, text) \
		GD_ASSERT_WINDOWS_DIALOG_ITEM_CREATE(0, x, y, cx, cy, id, 0x0082, text)
...
#pragma pack(push, 4)
	...
	struct AssertWindowsDialog final : public TNonCopyable
	{
		...
		WCHAR       m_DialogTitle[sizeof GD_ASSERT_DIALOG_TITLE] = GD_WIDEN(GD_ASSERT_DIALOG_TITLE);
		WORD        m_DialogFontSize = 8;
		WCHAR       m_DialogFont[sizeof "Verdana"] = L"Verdana";
		struct {
			GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE( 302,    7,  73,  14, ASSERT_DIALOG_BTN_ABORT,      GD_ASSERT_DIALOG_BTN_ABORT_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE( 302,   27,  73,  14, ASSERT_DIALOG_BTN_RETRY,      GD_ASSERT_DIALOG_BTN_RETRY_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE( 302,   47,  73,  14, ASSERT_DIALOG_BTN_IGNORE,     GD_ASSERT_DIALOG_BTN_IGNORE_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE( 302,   67,  73,  14, ASSERT_DIALOG_BTN_IGNORE_ALL, GD_ASSERT_DIALOG_BTN_IGNORE_ALL_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE( 302,   87,  73,  14, ASSERT_DIALOG_BTN_REPORT,     GD_ASSERT_DIALOG_BTN_REPORT_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_BUTTON_CREATE( 302,  107,  73,  14, ASSERT_DIALOG_BTN_BREAK,      GD_ASSERT_DIALOG_BTN_BREAK_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_TEXTEDIT_CREATE(11,   41, 283, 124, GD_ASSERT_WINDOWS_DIALOG_TEXTEDIT_ID, ""); // Would be set dynamically.
			GD_ASSERT_WINDOWS_DIALOG_INFOPANEL_CREATE(7,    4, 292, 165, /*ID, unused*/0,              GD_ASSERT_DIALOG_GROUPBOX_TEXT);
			GD_ASSERT_WINDOWS_DIALOG_STATICTEXT_CREATE(13, 17, 279,  20, /*ID, unused*/0,              GD_ASSERT_DIALOG_DESCRIPTION_TEXT);
		};
	...
	GDINT static INT_PTR CALLBACK AssertWindowsDialogProc(HWND const dlgHandle, UINT const dlgMessage, WPARAM const wParam, LPARAM const lParam)
	{
		...
		if (dlgMessage == WM_INITDIALOG)	// Dialog initialization.
		{
			...
			if (assertData->AssertExpression == nullptr)
			{
				...
				HWND const dlgButtonRetry = GetDlgItem(dlgHandle, GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + ASSERT_DIALOG_BTN_RETRY);
				...
				HWND const dlgButtonIgnore = GetDlgItem(dlgHandle, GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + ASSERT_DIALOG_BTN_IGNORE);
				...
				HWND const dlgButtonIgnoreAll = GetDlgItem(dlgHandle, GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + ASSERT_DIALOG_BTN_IGNORE_ALL);
...

			{
				...
				assert(SendMessageA(dlgHandle, DM_SETDEFID, static_cast<WPARAM>(GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + ASSERT_DIALOG_BTN_BREAK), 0));
#else	// if GD_DEBUG
				// In non-debug modes, debugger is not connected and we technically are unable to processed the
				// 'Break' button.
				HWND const dlgButtonBreak = GetDlgItem(dlgHandle, GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + ASSERT_DIALOG_BTN_BREAK);
				...
				assert(SendMessageA(dlgHandle, DM_SETDEFID, static_cast<WPARAM>(GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + ASSERT_DIALOG_BTN_REPORT), 0));
#endif	// if GD_DEBUG
			}
			...
			HWND const dlgTextEditWithMessage = GetDlgItem(dlgHandle, GD_ASSERT_WINDOWS_DIALOG_ITEM_ID + GD_ASSERT_WINDOWS_DIALOG_TEXTEDIT_ID);
		...
		else if (dlgMessage == WM_COMMAND)	// Dialog clicks handling.
		{
			...
			auto const dlgItemID = LOWORD(wParam) - GD_ASSERT_WINDOWS_DIALOG_ITEM_ID;
			if (dlgItemID >= 0 && dlgItemID <= ASSERT_DIALOG_BTNS_COUNT)
			{
