...
GD_NAMESPACE_BEGIN
		...
		, m_WindowBackgroundHandle(nullptr), m_WindowMainHandle(nullptr), m_WindowMainStateData{}, m_WindowOpenGL(nullptr)
	{
		...
		if (!windowClassesWereRegistered)
		{
			...
			GD_ASSERT(RegisterClass(&windowBackgroundClass) != 0, "'RegisterClass' (background window) function returned zero value with specified parameters.");
			...
			GD_ASSERT(RegisterClass(&windowMainClass) != 0, "'RegisterClass' (main window) function returned zero value with specified parameters.");
		}
		...
		GD_ASSERT(m_WindowBackgroundHandle != nullptr, "'CreateWindowEx' (background window) function returned invalid Handle.");
		...
		GD_ASSERT(m_WindowMainHandle != nullptr, "'CreateWindowEx' (main window) function returned invalid Handle.");
		...
		if ((windowFlags & WindowFlags::WithOpenGL) != 0)
		{
			...
			GD_ASSERT(m_WindowDeviceContext != nullptr, "'GetDC' (main window) function returned invalid Handle.");
			...
			GD_ASSERT(windowPixelFormat != 0, "'ChoosePixelFormat' error: failed to select correct pixel format.");
			...
			GD_ASSERT(m_WindowOpenGL != nullptr, "'wglCreateContext' (main window) function returned invalid Handle.");
	...
	GDAPI void WindowWindows::SetWindowMode(WindowMode const windowMode, bool const doForceReapply /*= false*/)
	{
		if (m_WindowMode != windowMode || doForceReapply)
		{
			...
			switch (windowMode)
			{
				...
				default: 
					GD_DEBUG_ASSERT_FALSE("Invalid window mode was specified.");
			}
