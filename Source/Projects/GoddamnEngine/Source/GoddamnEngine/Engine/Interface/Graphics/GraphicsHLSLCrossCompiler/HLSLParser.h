...
/*!
 * @file GoddamnEngine/Engine/Interface/Graphics/GraphicsHLSLCrossCompiler/HLSLParser.h
...
GD_NAMESPACE_BEGIN
	...
	public:
		...
		// ------------------------------------------------------------------------------------------
		//! Parses HLSL shader code into abstract syntax tree.
		GDAPI HLSLScope const* ParseShader(InputStream* inputStream);
	...
	GD_DSTRUCT(CopyFields)
	struct HLSLScope : public IUncopiable
	{
	public:
		Vector<HLSLDefinition const*> InnerDefinitions;

	public:
		GDINL virtual ~HLSLScope() { for (auto const Definition : InnerDefinitions) GD_DELETE(Definition); }
