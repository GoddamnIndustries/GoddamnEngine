...
GD_NAMESPACE_BEGIN
	...
	GDINT IGraphicsOpenGLBaseShader<IGraphicsShaderBase>::IGraphicsOpenGLBaseShader(IGraphicsShaderCreationInfo const* const gfxShaderCreationInfo)
		: IGraphicsShaderBase(gfxShaderCreationInfo), m_GLShaderProgramID(0), m_GLShaderProgramType(IGraphicsOpenGLShaderTypesTable[gfxShaderCreationInfo->gfxShaderType])
	{
		...
		if (!_HasCachedBinary)
		{
			...
			{
				BaseString<GLchar> glShaderCompilationLog(static_cast<UInt32>(glShaderCompilationLogLength));
