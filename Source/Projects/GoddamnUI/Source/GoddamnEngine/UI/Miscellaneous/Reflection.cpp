...
namespace Engine
{
	...
	{
		GetPropertyEnumerator::GetPropertyEnumerator(const string& property_name)
		{
			Result.Address = 0;
			Result.Type = PropertyType::Unknown;
			Result.Name = property_name;
		}
		void GetPropertyEnumerator::EnumerateProperty(const string& name, void* address, PropertyType type)
		{
			if (name == Result.Name)
			{
