...
/*!
 * @file GoddamnEngine/ShaderCrossCompiler/HLSLParser.cpp
...
#undef GD_HLSL_REGISTER_IN_SUPERGLOBAL_SCOPE
	...
	private:
		StreamedLexer      Lexer;
		Lexem              CurrentLexem;
		PtrDiffTp          AnonymousIdentifiersCount = -1;
	...
	{ 
		...
		{ 
			...
			Function->Arguments.Insert(Argument);
	...
	GDINT HLSLBinding const* HLSLParserImpl::ParseBinding(HLSLBindingType const ExpectedBindings)
	{
		...
		{ 
			...
			{ 
				...
				auto const RegisterIter = RegistersTranslationTable.FindIterator(*CurrentLexem.GetRawData().CStr());
	...
	{
		Lexer.GetNextLexem(&CurrentLexem);
	...
	{ 
		...
		auto const RequiredDefinitionIter = FindFirstIf(InnerDefinitions, [&DefinitionName](UniquePtr<HLSLDefinition const> const& Definition) -> bool
...
#undef GD_HLSL_DEFINE_SEMANTIC
		...
		auto const SemanticIndex = SemanticsList.FindIterator(Semantic.GetHashCode());
