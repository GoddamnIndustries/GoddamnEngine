...
GD_NAMESPACE_BEGIN
	...
	public:
		...
		/// @brief				Encodes string in BASE-64
		/// @param data			Data to encode
		/// @param output		Reference to resulting string
		GDAPI static void EncodeBase64(const Vector<byte>& data, String& output);

		/// @brief				Decodes string in BASE-64
		...
		/// @brief				Allocates sized string.
		/// @param _nullptr		Should be 'nullptr'
		/// @param size			Size for new String
		GDAPI String(const nullptr_t _nullptr = nullptr, const size_t size = 0, Char const fillWith = '\0');

		/// @brief				Copy constructor
		/// @param string		Copies from string.
		GDAPI String(const String& string);
		GDINL String(String&& string)
		{
		...
		/// @param size			New size
		GDAPI void Resize(const size_t size);
		...
		/// @endcode
		/// @returns			Trimmed from left-side string
		...
		/// @endcode
		/// @returns			Trimmed from right-side string
		...
		/// @brief				Searches for any instances of string and returns first index found
		/// @param string		String to search for
		/// @returns			First index found or '-1' if found nothing
		GDAPI size_t Find(const String& string) const;
		...
		/// @brief				Searches for any instances of string and returns last index found
		/// @param string		String to search for
		/// @returns			Last index found or '-1' if found nothing
		GDAPI size_t ReverseFind(const String& string) const;
		...
		GDAPI String& operator=	(const String& string);

		/// @brief			Concatenation operator
		GDAPI String operator+ (const String& string) const;

		/// @brief			Concatenation operator
		GDINL String operator+ (const Char* string) const;

		/// @brief			Concatenation operator
		GDINL String& operator+= (const String& string);

		/// @brief			Concatenation operator
		GDINL String& operator+= (const Char* string);
		...
		/// @brief			Comparing operator
		GDINL bool operator!= (const String& string) const;
...
#include <GoddamnEngine/Core/Containers/String/String.inl>
