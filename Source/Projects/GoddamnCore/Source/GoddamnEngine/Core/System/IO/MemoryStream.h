...
/*! 
 * @file GoddamnEngine/Core/System/IO/Stream.h
...
GD_NAMESPACE_BEGIN
	...
	public:
		...
		{
			GD_DEBUG_ASSERT(m_Memory != nullptr, "Invalid memory specified.");
		}
	...
	public:
		...
		GDINT virtual SizeTp Seek(PtrDiffTp const offset, SeekOrigin const origin = SeekOrigin::Current) override final
		{
			...
			switch (origin)
			{
				...
				case SeekOrigin::SeekEnd: 
					...
					break;
				default: 
					GD_DEBUG_ASSERT_FALSE("Invalid origin specified.");
			}
		...
		GDINT virtual SizeTp Read(Handle const array, SizeTp const size, SizeTp const count) override final
		{
			...
			CMemory::Memcpy(array, dataLength, m_Memory + m_MemoryPosition);
	...
	public:
		...
		GDINT virtual SizeTp Write(CHandle const array, SizeTp const size, SizeTp const count) override final
		{
			...
			CMemory::Memcpy(m_Memory.GetData() + memoryLocation, size * count, array);
			return count;
		}
