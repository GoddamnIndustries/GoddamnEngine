...
GD_NAMESPACE_BEGIN
	...
	public /*Constructors/Destructor*/:
		...
		{
			this->ConstructWithDataPointerAndSize(OtherBaseString.CStr(), OtherBaseString.GetLength());
		}
		...
		inline BaseString(size_t const Size, CharType const FillWith = CharType('\0'))
		{
			...
			(*(this->CStr() + this->GetLength())) = CharType('\0');
		}
	...
	public /*Class API*/:
		...
		GDINL MutableIterator End  ()       { return MutableIterator(this->CStr() + this->GetLength()); }
		GDINL ConstIterator   End  () const { return ConstIterator(this->CStr() + this->GetLength()); }

		/// Returns iterator that points to last container element.
		GDINL ReverseMutableIterator ReverseBegin()       { return ReverseMutableIterator(MutableIterator(this->CStr() + (this->GetLength() - 1))); }
		GDINL   ReverseConstIterator ReverseBegin() const { return   ReverseConstIterator(ConstIterator(this->CStr() + (this->GetLength() - 1))); }
		...
		GDINL bool IsEmpty() const { return (this->GetLength() != 0); }
		...
		GDINL CharType  GetLastElement() const { return (*(this->CStr() + this->GetLength() - 1)); }
		...
		{
			return this->GetSubstring(From, this->GetLength());
		}
	...
	public /*Operators*/:
		...
		{
			BaseString Result(this->StringSize + OtherBaseString.GetLength());
