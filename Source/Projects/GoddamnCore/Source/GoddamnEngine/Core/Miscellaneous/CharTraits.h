...
#	define GD_TEXT(Text) Text
	...
	struct CharTraitsBase<CharAnsi>
	{
		GDINL static CharAnsi ToLower(CharAnsi const Character)
		{ 
			return static_cast<CharAnsi>(::tolower(Character)); 
		}

		GDINL static CharAnsi ToUpper(CharAnsi const Character)
		{ 
			return static_cast<CharAnsi>(::toupper(Character)); 
		}
		...
		GDINL static CharAnsi const* StrRChr(CharAnsi const* const CString, CharAnsi const Character)
		{ 
			return ::strrchr(CString, Character); 
		}
		...
		GDINL static int StrNCmp(CharAnsi const* const First, CharAnsi const* const Second, size_t const MaxLength)   
		{ 
			return ::strncmp(First, Second, MaxLength); 
		}

		GDINL static int VSNPrintF(CharAnsi* CString, size_t const Count, CharAnsi const* const Format, va_list Args)		   
		{ 
			return ::vsnprintf(CString, Count, Format, Args); 
		}
	};	// struct CharTraitsBase<CharAnsi>
	...
	{
		GDINL static CharUnicode ToLower(CharUnicode const Character)
		{
			return static_cast<CharUnicode>(::towlower(Character)); 
		}

		GDINL static CharUnicode ToUpper(CharUnicode const Character)
		{ 
			return static_cast<CharUnicode>(::towupper(Character)); }

		GDINL static size_t StrLen(CharUnicode const* const CString)
		{ 
			return ::wcslen(CString); }

		GDINL static CharUnicode const* StrChr(CharUnicode const* const CString, CharUnicode const Character)
		{ 
			return ::wcschr(CString, Character); 
		}

		GDINL static CharUnicode const* StrRChr(CharUnicode const* const CString, CharUnicode const Character)
		{ 
			return ::wcsrchr(CString, Character); 
		}

		GDINL static CharUnicode const* StrStr(CharUnicode const* const First, CharUnicode const* const Second)
		{ 
			return ::wcsstr(First, Second); 
		}

		GDINL static int StrNCmp(CharUnicode const* const First, CharUnicode const* const Second, size_t const MaxLength)
		{ 
			return ::wcsncmp(First, Second, MaxLength); 
		}

		GDINL static int VSNPrintF(CharUnicode* CString, size_t const Count, CharUnicode const* const Format, va_list Args)
		{ 
			return ::vswprintf(CString, Count, Format, Args); 
		}
	};	// struct CharTraitsBase<CharUnicode>
#endif	// if (defined(GD_DOCUMENTATION))

	/// @brief Provides helper functions for processing ANSI characters.
	template<typename CharType>
	struct CharTraits final : public CharTraitsBase<CharType>
	{
		/// @brief Returns true if this character is valid digit in specified notation.
		...
		/// @brief Converts this character in specified notation to decimal one.
		///        Currently supports notations range between 2 and 36.
