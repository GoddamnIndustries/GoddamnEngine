...
/*! 
 * @file GoddamnEngine/Core/System/IO/FileSystemWatcherWindows.cpp
...
GD_NAMESPACE_BEGIN
		...
		, m_Filter(filter), m_DirectoryHandle(nullptr), m_Recursive(recursive)
	{
		...
		GD_ASSERT(m_DirectoryHandle != INVALID_HANDLE_VALUE, "'CreateFile' function has failed.");
					...
					switch (notifyInformation->Action)
					{
						...
						case FILE_ACTION_RENAMED_OLD_NAME:
							{	// Renaming action sends two notifications: first with old name, and second with the new one.
								...
								GD_DEBUG_ASSERT(notifyInformation->Action == FILE_ACTION_RENAMED_NEW_NAME, "Unknown changes buffer layout for renaming.");
							}
