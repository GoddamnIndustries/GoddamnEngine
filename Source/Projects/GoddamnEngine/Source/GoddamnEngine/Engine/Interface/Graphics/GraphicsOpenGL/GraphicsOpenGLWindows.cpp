...
/*!
 * @file GoddamnEngine/Engine/Interface/Graphics/GraphicsOpenGL/GraphicsOpenGLWindows.cpp
 * File contains Implementations for Windows-specific code of the OpenGL 4.3 Implementation 
 * of the graphics interface.
 */
#include <GoddamnEngine/Engine/Interface/Graphics/GraphicsOpenGL/GraphicsOpenGLWindows.h>
...
GD_NAMESPACE_BEGIN
	...
	GDAPI IResult IGraphicsOpenGLWindows::OnRuntimePreInitialize()
	{
		...
		// Retrieving information about the list of supported screen resolutions.
		{
			...
			for (DWORD _Cnt = 0; EnumDisplaySettings(nullptr, _Cnt, &HGLRCTestCanvasMode) != GD_FALSE; ++_Cnt)
			{
				...
				{
					GfxResolutionsList.InsertAt(0, { static_cast<UInt32>(HGLRCTestCanvasMode.dmPelsWidth), static_cast<UInt32>(HGLRCTestCanvasMode.dmPelsHeight), 30, 1 });
					GfxResolutionsList.InsertAt(0, { static_cast<UInt32>(HGLRCTestCanvasMode.dmPelsWidth), static_cast<UInt32>(HGLRCTestCanvasMode.dmPelsHeight), 60, 1 });
				}
			}
		}
	...
	GDAPI IResult IGraphicsOpenGLWindows::OnRuntimeDeinitialize()
	{
		...
		ReleaseDC(hwndMain, HGLRCDeviceContext);
