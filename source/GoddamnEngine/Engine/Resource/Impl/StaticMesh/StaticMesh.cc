...
GD_NAMESPACE_BEGIN
	...
	void LoadShekel_01(InputStream* const InputResourceData, HRIIndexedShape* const staticMesh)
	{
		...
		InputResourceData->ReadCheck(&verticesNumber, 0, sizeof(verticesNumber));

		UInt32 indicesNumber = 0;
		InputResourceData->ReadCheck(&indicesNumber, 0, sizeof(indicesNumber));

		Vector<Float32> verticesBuffer;
		verticesBuffer.Resize(static_cast<size_t>(verticesNumber) * sizeof(Vector3) / sizeof(Float32));

		InputResourceData->ReadCheck(&verticesBuffer[0], 0, sizeof(Vector3) * static_cast<size_t>(verticesNumber));
		...
		InputResourceData->ReadCheck(&verticesBuffer[0], 0, sizeof(Vector2) * static_cast<size_t>(verticesNumber));
		...
		InputResourceData->ReadCheck(&indicesBuffer[0], 0, static_cast<size_t>(indicesNumber)* sizeof(UInt16));
	...
	void StaticMesh::OnResourceLoad(InputStream* const InputResourceData)
	{
		...
		InputResourceData->ReadCheck(&shekelHeader, 0, sizeof(shekelHeader));
