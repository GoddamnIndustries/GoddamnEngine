...
GD_NAMESPACE_BEGIN
	...
	{	// Parsing shader source.
		...
		try {
			UniquePtr<HLSLScope const>    const ShaderParsedData(HLSLParser(self->Toolchain).ParseShader(Forward<UniquePtr<InputStream>>(ShaderSourceInputStream)));
			RefPtr<HRIShaderInstanceDesc> const ShaderInstanceDesc(HLSLValidator(self->Toolchain).ValidateAndGenerateDescription(ShaderParsedData.GetPointer(), ShaderEntryPointName));
			...
			if (ShaderByteCodeOutputStream != nullptr) {	// If required, compiling shader.
				switch (ShaderTargetPlatform) {
					...
					case GD_HRI_SHADERCC_COMPILER_TARGET_HLSL:
						HLSLCompiler(self->Toolchain).GenerateAndCompileShader(ShaderByteCodeOutputStream, ShaderParsedData.GetPointer(), ShaderType, ShaderEntryPointName);
						break;
#endif	// if (defined(GD_PLATFORM_WINDOWS))

#if (defined(GD_PLATFORM_OSX))
					case GD_HRI_SHADERCC_COMPILER_TARGET_METAL:
						MetalCompiler(self->Toolchain).GenerateAndCompileShader(ShaderByteCodeOutputStream, ShaderParsedData.GetPointer(), ShaderType, ShaderEntryPointName);
						break;
#endif	// if (defined(GD_PLATFORM_OSX))
					...
					case GD_HRI_SHADERCC_COMPILER_TARGET_GLSLES3:
						GLSLCompiler(self->Toolchain).GenerateAndCompileShader(ShaderByteCodeOutputStream, ShaderParsedData.GetPointer(), ShaderType, ShaderEntryPointName, ShaderTargetPlatform);
						break;
