...
GD_NAMESPACE_BEGIN
	...
	public:
		...
		{
			GD_VERIFY(IsValid(), "Getting position of the invalid stream.");
		...
		{
			GD_VERIFY(IsValid(), "Getting length of the invalid stream.");
		...
		GDINT virtual void Close() override final
		{
			...
			{
				GD_VERIFY(CStdio::Fclose(m_FileHandle) == 0, "'CStdio::Fclose' has failed.");
		...
		{
			GD_VERIFY(IsValid(), "Seeking in invalid stream.");
		...
		{
			GD_VERIFY(IsValid(), "Reading from invalid stream.");
	...
	public:
		...
		{
			GD_VERIFY(IsValid(), "Flushing invalid stream.");
