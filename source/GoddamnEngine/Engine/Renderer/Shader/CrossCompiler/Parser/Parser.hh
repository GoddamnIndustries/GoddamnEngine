...
GD_NAMESPACE_BEGIN
	...
	struct HLSLDefinition : public HLSLExpression
	{
		...
		GDINL explicit HLSLDefinition(HLSLDefinition const& Definition, HLSLDefinitionType const DefinitionType = GD_HLSL_DEFINITION_UNKNOWN) : HLSLExpression(GD_HLSL_EXPRESSION_DEFINITION), DefinitionType(DefinitionType)
		{
	...
	public:
		...
		GDINL explicit HLSLScope() { for (auto const Definition : self->InnerDefinitions) delete Definition; }
	...
	struct HLSLVariable : public HLSLDefinition
	{
		...
		GDINL explicit HLSLVariable(                                ) : HLSLDefinition(            GD_HLSL_DEFINITION_VARIABLE) { delete self->ExprColon; delete self->InitialValue; }
		GDINL explicit HLSLVariable(HLSLDefinition const& Definition) : HLSLDefinition(Definition, GD_HLSL_DEFINITION_VARIABLE) { }
		GDINL explicit HLSLVariable(HLSLVariable       && Variable  ) : HLSLDefinition(Variable,   GD_HLSL_DEFINITION_ARGUMENT) 
		{
			self->ExprColon    = Variable.ExprColon;
			self->InitialValue = Variable.InitialValue;
			self->ArrayIndex   = Variable.ArrayIndex;
	...
	struct HLSLFunction final : public HLSLDefinition
	{
		...
		GDINL virtual ~HLSLFunction(                                ) { for (auto const Argument : self->Arguments) delete Argument; delete self->Semantic; }
		GDINL explicit HLSLFunction(                                ) : HLSLDefinition(            GD_HLSL_DEFINITION_FUNCTION) { }
		GDINL explicit HLSLFunction(HLSLDefinition const& Definition) : HLSLDefinition(Definition, GD_HLSL_DEFINITION_FUNCTION) { }
	};	// struct HLSLFunction
	...
	public /*Class API*/:
		...
		GDAPI HLSLScope const* ParseShader(Stream* const InputStream);
