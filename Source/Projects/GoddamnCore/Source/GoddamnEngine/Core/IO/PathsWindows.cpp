...
/*! 
 * @file GoddamnEngine/Core/IO/PathsWindows.cpp
...
#pragma warning(default: 4047)
	...
	GDAPI WideString const& PathsWindows::GetEngineSavedDataUserDirectory()
	{
		...
		if (engineSavedDataUserDirectory.IsEmpty())
		{
			...
			else
			{
				...
				GD_VERIFY(SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, nullptr, SHGFP_TYPE_CURRENT, engineSavedDataUserDirectoryBuilder))
	...
	GDAPI WideString const& PathsWindows::GetApplicationExecutableFile()
	{
		...
		if (applicationExecutableFile.IsEmpty())
		{
			...
			GD_VERIFY(GetModuleFileNameW(nullptr, applicationExecutablePath, GD_ARRAY_LENGTH(applicationExecutablePath) - 1) != 0
				, "'GetModuleFileName' function has failed.");
