...
GD_NAMESPACE_BEGIN
	...
	//	for (GDInt64 cnt = (GDInt64)data.GetLength(); cnt >= 0; cnt -= 1)
	//	{
	//	char3[i++] = *(dataPointer++);
	//	if (i == 3)
	//	{
	...
	String::String(const nullptr_t _nullptr, const size_t HeapSize, Char const fillWith)
	{
		...
		this->HeapMemory	= (Char*)Allocator::AllocateMemory((this->GetLength() + 1) * sizeof(Char));

		memset(this->HeapMemory, (int)fillWith, (this->GetLength()) * sizeof(Char));
		*this->End() = '\0';
	}
	...
	String::String(const CharAnsi* HeapMemory)
	{
		...
		memcpy(this->HeapMemory, HeapMemory, (this->GetLength() + 1) * sizeof(Char));
	}
	...
	String::String(const CharUtf16* HeapMemory)
	{
		...
		wcstombs(this->HeapMemory, HeapMemory, (this->GetLength() + 1) * sizeof(Char));
	}
	...
	String::String(const CharUtf32* HeapMemory)
	{
		...
		{
			VertexBufferObject[this->GetLength()] = (Char)HeapMemory[this->GetLength()];
		}

		this->HeapMemory = (Char*)Allocator::AllocateMemory((this->GetLength() + 1) * sizeof(Char));
		memcpy(this->HeapMemory, &VertexBufferObject[0], (this->GetLength() + 1) * sizeof(Char));
	}

	String::String(const String& HeapMemory)
	{
		this->HeapSize		= HeapMemory.GetLength();
		this->HeapMemory	= (Char*)Allocator::AllocateMemory((this->GetLength() + 1) * sizeof(Char));

		memcpy(this->HeapMemory, &HeapMemory[0], (this->GetLength() + 1) * sizeof(Char));
	}
	...
	String String::TrimmLeft() const
	{
		...
		size_t index = 0;
		for (; (index < this->GetLength() && (spaces.Find((*this)[index]) != -1)); index += 1);
	...
	size_t String::ReverseFind(const String& HeapMemory) const
	{
		...
		return (foundIndex != -1 ? (this->GetLength() - foundIndex) : -1);
	}
	...
	HashCode String::GetHashCode() const
	{
		...
		for (size_t cnt = 0; cnt < this->GetLength(); cnt += 1) 
		{
	...
	{
		this->Resize(HeapMemory.GetLength());

		memcpy(&(*this)[0], &HeapMemory[0], this->GetLength());
	...
	{
		String result(nullptr, this->GetLength() + HeapMemory.GetLength());

		memcpy(&result[0], (&this->HeapMemory[0]), this->GetLength() * sizeof(Char));
		memcpy(&result[this->GetLength()], &HeapMemory[0], (HeapMemory.GetLength() + 1) * sizeof(Char));
