...
struct IteratorTagConst { bool const static IsConst = true; };
struct IteratorTagMutable { bool const static IsConst = false; };
	...
	public:
		...
		template<typename ObjectType> using        MutableIterator =        Iterator<IteratorTagMutable, ObjectType>;
	...
	public:
		...
		///	is incremented, when finished - is decremented. When counter becomes zero, objects destroys itthis.
	...
	{
		static_assert(TypeTraits::IsBase<Object, ObjectType>::Value, "'ObjectType' should be derived from 'Object'");
	public /*Inner types*/:
