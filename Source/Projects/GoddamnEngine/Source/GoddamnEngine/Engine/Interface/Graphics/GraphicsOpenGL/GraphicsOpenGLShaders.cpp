...
GD_NAMESPACE_BEGIN
	...
	GDINT IGraphicsOpenGLBaseShader<IGraphicsShaderBase>::IGraphicsOpenGLBaseShader(IGraphicsShaderCreationInfo const* const gfxShaderCreationInfo)
		: IGraphicsShaderBase(gfxShaderCreationInfo), GLShaderProgramID(0), GLShaderProgramType(IGraphicsOpenGLShaderTypesTable[gfxShaderCreationInfo->gfxShaderType])
	{
		...
		if (!_HasCachedBinary)
		{
			...
			GLchar const* glShaderSources[] = {
				...
				static_cast<GLchar const*>(gfxShaderCreationInfo->gfxShaderByteCode),
				"\n"
			};
			...
			//if (CString::Strstr(glVendor, "ATI") != nullptr)
			{
				...
				glShaderSources[1] = glShaderSourceATIInsetions[gfxShaderCreationInfo->gfxShaderType];
			}

			GLShaderProgramID = glCreateShaderProgramv(GLShaderProgramType, GD_ARRAY_LENGTH(glShaderSources), glShaderSources);
#if !GD_RELEASE
			...
			glGetProgramiv(GLShaderProgramID, GL_INFO_LOG_LENGTH, &glShaderCompilationLogLength);
			if (glShaderCompilationLogLength > 1)
			{
				...
				glGetProgramInfoLog(GLShaderProgramID, static_cast<GLsizei>(glShaderCompilationLogLength), nullptr, glShaderCompilationLog.CStr());
				ConsoleDevice->LogFormat("Validation of the separate shader object of type %d proceeded with the following log: \n%s"
					, gfxShaderCreationInfo->gfxShaderType, glShaderCompilationLog.CStr());
		...
		, SizeTp const gfxUniformBuffersCount)
	{
		...
		for (GLuint glUniformBlockIndex = 0; glUniformBlockIndex < gfxUniformBuffersCount; ++glUniformBlockIndex)
		{
			if (glUniformBuffers != nullptr)
			{
				...
				glBindBufferBase(GL_UNIFORM_BUFFER, gfxUniformBufferBindingPointOffset + glUniformBlockIndex, glUniformBuffer->GLBufferID);
			}
			else
			{
		...
		, SizeTp const gfxResourcesCount)
	{
		...
		for (GLuint glResourceIndex = 0; glResourceIndex < gfxResourcesCount; ++glResourceIndex)
		{
			...
			if (glResources != nullptr)
			{
				...
				glBindTexture(glResource->GLShaderResourceType, glResource->GLShaderResourceID);
			}
			else
			{
		...
		, SizeTp const gfxSamplerStatesCount)
	{
		...
		for (GLuint glSamplerIndex = 0; glSamplerIndex < gfxSamplerStatesCount; ++glSamplerIndex)
		{
			if (glSamplerStates != nullptr)
			{
				...
				glBindSampler(gfxSamplerBindingPointOffset + glSamplerIndex, gfxSamplerState->GLSamplerStateID);
			}
			else
			{
