...
GD_NAMESPACE_BEGIN
	...
	{
		Vector<UInt8> texture2DData(InputResourceData->GetLength());
		InputResourceData->Read(&texture2DData[0], 0, texture2DData.GetLength());
		/*std::ostringstream texture2DStream;
		texture2DStream.write(
			reinterpret_cast<char const*>(&texture2DData[0]),
			texture2DData.GetLength() / sizeof(char)
		);*/
		...
		this->Texture = HRInterface::GetInstance().CreateTexture2D(
			...
			&texture2DPixels[0], texture2DPixels.GetLength()
		);
	}
