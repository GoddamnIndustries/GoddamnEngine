...
GD_NAMESPACE_BEGIN
	...
	{	
		if (   (this->Options.SingleLineCommentDeclaration[this->CurrentMatchingIndex] == this->CurrentCharacter)
			...
			if (this->Options.SingleLineCommentDeclaration.GetSize() == this->CurrentMatchingIndex) {	// Final match, finalizing comment.
				this->CurrentMatchingIndex = 0;
			...
			if (this->Options.MultipleLineCommentBeginning.GetSize() == this->CurrentMatchingIndex)	{	// Final match, finalizing comment.
				this->CurrentMatchingIndex = 0;
	...
	void BasicStreamedLexerImpl::ProcessMultipleLineComment()
	{
		if (this->Options.MultipleLineCommentEnding[this->CurrentMatchingIndex] == this->CurrentCharacter) {	// Matching with multiple line comment ending.
			this->CurrentMatchingIndex += 1;
			if (this->Options.MultipleLineCommentEnding.GetSize() == this->CurrentMatchingIndex) {	// Final match, finalizing comment.
				this->CurrentMatchingIndex = 0;
				this->WasLexemCommited = true;
				return;
			}
	...
	{
		if (this->CurrentMatches.GetSize() == 0) {	// Filling CurrentMatches with pointers on operators and comments declarations
			this->CurrentMatchingIndex = 0;
		...
		for (auto const CurrentOperatorOrCommentMatch : this->CurrentMatches) {	
			if ((*CurrentOperatorOrCommentMatch)[this->CurrentMatchingIndex] == this->CurrentCharacter)	{	// Character matches some declaration at current index.
				if (CurrentOperatorOrCommentMatch->GetSize() == (1 + this->CurrentMatchingIndex)) {	// We are having a full match with some operator, so lets identify it now
		...
		if (CurrentMatches.GetSize() == 0) {	// No matches in this iteration. That means that currect operator ID was set in previos. 
	...
	{
		if ((this->CurrentMatches.GetSize() == 0) && (this->CurrentMatchingIndex == 0))	{	// Filling CurrentMatches with pointers on operators and comments declarations
		...
		for (auto const CurrentIdentifierMatch : this->CurrentMatches) {	
			if ((*CurrentIdentifierMatch)[this->CurrentMatchingIndex] == this->CurrentCharacter) {	// Character matches some declaration at current index
				if (CurrentIdentifierMatch->GetSize() == (1 + this->CurrentMatchingIndex)) {	// We are having a full match with some keyword, so lets identify it now.
