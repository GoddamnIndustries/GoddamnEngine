        ...
        public sealed override string GenerateProjectFiles(ProjectCache Project)
        {
            ...
            using (XmlTextWriter VCXProj = new XmlTextWriter(VCXProjPath, null) { Formatting = Formatting.Indented, Indentation = 1, IndentChar = '\t' }) {
                ...
                foreach (TargetPlatform Platform in Target.EnumerateAllPlatforms()) {
                    ...
                    // Defining macros..
                    string MacrosPlatform = Project.GenerateMacros(Platform);
                    ...
                    string LinkedLibraries = Project.GenerateLinkedLibrariesPaths(Platform);
                    ...
                    foreach (TargetConfiguration Configuration in Target.EnumerateAllConfigurations()) {
                        ...
                        string ConfigurationName = string.Concat(Configuration, Platform);
                        string OutputPath = Project.m_CachedOutputPathDelegates[Platform](Configuration);
                        string Macros = MacrosPlatform;
                        ...
                        if (IsPlatformNativelySupported(Platform)) {
                            ...
                            if (!ConfigurationInfo.m_GenerateDebugInformation) {
                                VCXProj.WriteElementString("GenerateDebugInformation", "false");
                            }
            ...
            using (StreamWriter SLN = new StreamWriter(SLNPath)) {
                ...
                foreach (var SolutionProject in Solution.m_CachedProjects) {
                    ...
                    if (SolutionProject.m_IsBuildTool) {
                        ...
                        // in Visual Studio when reloading of solution, and all opened insets would be terminated..
                        try {
                            XmlDocument CSProj = new XmlDocument();
                            XmlNamespaceManager CSProjNamespaceMgr = new XmlNamespaceManager(CSProj.NameTable);
                            CSProjNamespaceMgr.AddNamespace("msb", s_MSBuild2003Namespace);
                            CSProj.Load(SolutionProject.m_GeneratorOutputPath);

                            XmlNode CSProjGUIDNode = CSProj.SelectSingleNode("//msb:PropertyGroup/msb:ProjectGuid", CSProjNamespaceMgr);
                            SolutionProject.m_AdditionalCache.m_GUID = CSProjGUIDNode.InnerText;
                ...
                foreach (var SolutionProject in Solution.m_CachedProjects) {
                    ...
                    SLN.WriteLine("Project(\"{0}\") = \"{1}\", \"{2}\", \"{3}\"",
                        /* 0 */ SolutionProject.m_IsBuildTool ? s_SLNCSProjProjectGUID : s_SLNVCProjProjectGUID,
