...
/*! 
 * @file GoddamnEngine/Core/System/IO/DirectoryWindows.h
 * File contains Windows directory interface.
 */
#pragma once
#if !defined(GD_INSIDE_DIRECTORY_H)
#	error This file should be never directly included, please consider using <GoddamnEngine/Core/System/IO/Directory.h> instead.
...
GD_NAMESPACE_BEGIN
	...
	public:
		...
		GDAPI void Close()
		{
			...
			{
				GD_ASSERT(FindClose(m_DirHandle) == TRUE, "'FindClose' has failed.");
		...
		GDAPI void Rewind()
		{
			...
			if (WideCString::Strlen(m_DirPath) <= MAX_PATH - 3)
			{
				...
				WideCString::Strcpy(directoryPathWin, GD_ARRAY_LENGTH(directoryPathWin), m_DirPath);
				WideCString::Strcat(directoryPathWin, GD_ARRAY_LENGTH(directoryPathWin), L"\\*");
		...
		{
			GD_DEBUG_ASSERT(IsValid(), "Reading from invalid directory visitor.");
			if (m_DirFirstRead)
			{
